...
impl Args {
    pub fn exec(self) -> miette::Result<()> {
        let arena = pallas_uplc::bumpalo::Bump::with_capacity(1_024_000);
        ...
        let program = if self.flat {
            pallas_uplc::flat::decode(&arena, &program).into_diagnostic()?
        } else {
            {
            ...
            let parse_result =
                pallas_uplc::syn::parse_program(&arena, &program_string).into_result();
        ...
        for (index, arg) in self.args.iter().enumerate() {
            let parse_result = pallas_uplc::syn::parse_term(&arena, arg).into_result();
